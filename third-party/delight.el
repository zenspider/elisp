...
;; immediate effect upon calling â€˜delightâ€™.  Major mode names are held in
...
;;
;; Major modes based on cc-mode.el (including â€˜c-modeâ€™, â€˜c++-modeâ€™, and
;; derivatives such as â€˜php-modeâ€™) cannot be delighted, due to Emacs bug
;; #2034: https://debbugs.gnu.org/cgi/bugreport.cgi?bug=2034
;;
;; cc-mode.el assumes that â€˜mode-nameâ€™ is always a string (which was true
;; in Emacs 22 and earlier), while delight.el makes use of the fact that
;; â€˜mode-nameâ€™ can (since Emacs 23) contain any mode line construct.  The
...
;;
;; The conflicting function is â€˜c-update-modelineâ€™ which adds the various
;; suffix characters documented at M-: (info "(ccmode) Minor Modes").
;; (E.g. In the mode line of a â€˜c-modeâ€™ buffer, the name C might be
...
;; conflict between the two libraries by disabling â€˜c-update-modelineâ€™
;; entirely, like so:
;;
